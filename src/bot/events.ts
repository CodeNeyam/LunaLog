...
export function registerBotEvents(client: Client, deps: BotDeps): void {
  ...
  client.once("ready", () => {
    logger.info("Bot ready", { user: client.user?.tag ?? "unknown" });
  });

  client.on("guildMemberAdd", async (member) => {
    try {
      if (!member || member.user?.bot) return;
      const joinIso = member.joinedAt ? member.joinedAt.toISOString() : new Date().toISOString();
      statements.users.upsertUser(member.id, joinIso);
      await momentsService.ensureMoment(member.id, "JOINED", { guildId: member.guild.id }, joinIso);
    } catch (err) {
      logger.error("guildMemberAdd handler failed", { err: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      logger.error("messageCreate handler failed", { err: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  client.on("voiceStateUpdate", async (oldState, newState) => {
    try {
      ...
      logger.error("voiceStateUpdate handler failed", { err: err instanceof Error ? err.message : String(err) });
    }
  });
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      logger.error("interactionCreate handler failed", { err: err instanceof Error ? err.message : String(err) });
      try {
